...
export function makeMath(addends: number[]): string {
    const sum = addends.reduce((ac, cur) => ac + cur, 0);
...
export function injectPositive(values: number[]): number[] {
    const negative = (arr: number[], index: number) =>
        arr.slice(0, index).reduce((ac, cur) => ac + cur, 0);
    const negativeID = values.findIndex((num) => num < 0);
    if (negativeID === -1) {
        return [...values, negative(values, values.length)];
    } else {
        const sum = negative(values, negativeID);
        return [
            ...values.slice(0, negativeID + 1),
            sum,
            ...values.slice(negativeID + 1),
        ];
    }
}
